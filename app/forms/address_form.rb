...
class AddressForm < Rectify::Form
  ...
  def init_with_params(params)
    ...
    @billing_address = AddressFieldsForm.from_params(params[:billing_form])
    if @use_billing
      @shipping_address = AddressFieldsForm.from_params(params[:billing_form])
      @shipping_address.address_type = :shipping
    else
      @shipping_address = AddressFieldsForm.from_params(params[:shipping_form])
    end
  end
